...
const ProductTable = () => {
  ...
  const handleEditClick = (productId) => {
    const updated = filteredProducts.map((product) =>
      product.productId === productId ? { ...product, isEditing: true } : product
    );
    setFilteredProducts(updated);
    setProducts(
      products.map((p) =>
        p.productId === productId ? { ...p, isEditing: true } : p
      )
    );
  };

  const handleDoneClick = async (productId, newPrice, newQuantity) => {
    try {
