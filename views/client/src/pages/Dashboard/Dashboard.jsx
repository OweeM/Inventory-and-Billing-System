...
// Component for displaying stats cards - reduced padding
...
// Compact inventory overview component
const InventoryOverview = ({ inventoryData }) => (
  <div>
    <h2 className="text-md font-bold text-gray-700 mb-2">Inventory Overview</h2>
    <div className="grid grid-cols-1 gap-2">
      {[
        { title: "Total Products", value: inventoryData.totalProducts },
        { title: "Low Stock Items", value: inventoryData.lowStockItems },
        { title: "Out of Stock Items", value: inventoryData.outOfStockItems },
        { title: "New Products", value: inventoryData.newProducts },
      ].map((item, index) => (
        <div
          key={index}
        ...
        >
          ...
          <h2 className="text-sm font-bold text-gray-800">{item.value}</h2>
        </div>
      ))}
    </div>
  </div>
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const { stats, inventory, recentBills } = response.data;
        setDashboardData({
          stats: {
            dailySales: stats.dailySales || 0,
            customersCount: stats.customersCount || 0,
            dailyTransactions: stats.dailyTransactions || 0,
          ...
          inventory: {
            ...
            newProducts: inventory.newProducts || 0
          },
          recentBills: Array.isArray(recentBills) ? recentBills : []
        });
        setError(null);
      } catch (err) {
  ...
  return (
    <div className="flex bg-gray-100 h-screen overflow-hidden">
      ...
      <div className="flex-1 p-4 overflow-auto">
        ...
        {/* Graph and Inventory Overview - reduced gap and padding */}
        <div className="flex flex-col lg:flex-row gap-3 mb-3">
          {/* Graph Section - reduced height */}
          <div className="flex-1 bg-white p-3 rounded-md shadow">
            <h2 className="text-md font-bold text-gray-700 mb-2 text-center">Daily Sales Revenue by Product</h2>
          ...
          {/* Inventory Overview - reduced width */}
          <div className="w-full lg:w-1/4 bg-white p-3 rounded-md shadow">
            <InventoryOverview inventoryData={dashboardData.inventory} />
          </div>
        </div>

        {/* Recent Bills Table - fixed height with internal scroll */}
        <div className="bg-white p-3 rounded-md shadow">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-md font-bold text-gray-700">Recent Bills Processed</h2>
            <span className="text-xs text-gray-500">
              {dashboardData.recentBills.length} bills found
            </span>
          </div>
